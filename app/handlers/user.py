...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"💡 Xarajat qo'shish uchun shunchaki matn yuboring:\n"
        f"Masalan: \n`Non 4 ta 3000` yoki `Non 4 ta 3 000`\n`Kola 15 000`",
...
# ================= DAROMAD KIRITISH (/kirim) =================
@user_router.message(F.text.in_({"➕ Daromad kiritish", "➕ Добавить доход", "➕ Add Income"}) | (F.text == "/kirim"))
async def process_income_btn(message: types.Message, state: FSMContext):
    await state.set_state(FSM.income)
    await message.answer("💰 Daromad summasini kiriting (masalan: `150000` yoki `15 000 000`):", 
...
async def process_income_amount(message: types.Message, state: FSMContext):
    ...
    if not text.replace('.', '', 1).isdigit():
        await message.answer("❌ Noto'g'ri qiymat. Faqat raqam kiriting (masalan: `150000` yoki `15 000 000`):", reply_markup=get_cancel_kb(), parse_mode="Markdown")
        return
...
@user_router.message(F.text.in_({"📊 Diagramma", "📊 Диаграмма", "📊 Chart"}) | (F.text == "/report"))
